...
- Receivers must treat a large `frame_id`/`message_id` regression (more than
  1000 below the current id) as a sender restart and start a new stream;
  small regressions remain stale packets. Without this a restarted onboard
  process is rejected until the GCS also restarts.
- Receivers should enlarge the UDP receive buffer (>= 1 MB recommended):
