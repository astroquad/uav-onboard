...
초기 video stream은 MJPEG frame을 UDP chunk로 나누어 전송한다. 각 chunk는
`AQV1` magic, frame id, chunk index/count, payload size, timestamp를 포함한
고정 header 뒤에 JPEG payload를 붙인다. 수신 측은 완성되지 않은 frame을
폐기하고 최신 완성 frame 표시를 우선한다.
