...
| Safety monitor / vision-stale watchdog | 구현됨(실비행으로 계속 검증) | `src/safety/SafetyMonitor.*`, `src/app/AstroquadOnboardApp.*` |
...
| 실행별 flight logging | 구현됨 | `src/logging/FlightLogger.*`, `config/logging.toml` |
| Flight-log 전용 replay 도구 | 미구현 | future work |
...
- 과거 `line_follow_node`의 `ALT_HOLD + RC_CHANNELS_OVERRIDE` 실험 경로는
  EKF 위치 안정화 우회, RC failsafe 충돌, 구 1.5kg/3S 기체 전용 PWM 상수
  의존 때문에 2026-07에 폐기되었다.
- 현 2.1kg/4S 기체에서 실행하지 않는다. 지원 경로는 GUIDED body-frame
  velocity 하나다.
...
현재 문서 version은 v1.13이고 JSON top-level `protocol_version`은 호환성을
위해 integer `1`이다.
...
[debug_video]
enabled = false
send_fps = 12      # LTE-sized 기본 상한 (--fps로 override)
jpeg_quality = 55
chunk_pacing_us = 150
send_width = 600   # GCS debug video downscale (0 = 카메라 JPEG 그대로)
send_height = 0    # 0 = 종횡비 유지
fec_group_size = 4 # 데이터 청크 4개마다 XOR parity 1개
```

실기체 runtime의 debug video는 sender worker thread에서 기본 600px 폭,
quality 55, 12fps로 downscale/재인코딩되고 XOR FEC를 붙인다. Frame telemetry는
기본 6fps다. FEC 포함 약 2.1~2.2 Mbit/s를 목표로 한 LTE-sized 설정이며,
Vision/mission loop는 latest-wins submit 비용만 낸다. `send_fps = 0`은 처리
프레임을 전부 보내는 LAN용 값이고, 양수 값(또는 `--fps <n>`)은 상한을 건다
(camera fps=24로 clamp). Telemetry 좌표는 camera pixel space를 유지하고 GCS가
overlay를 스케일한다.
...
send FPS. Frame telemetry follows the configured 6fps cap by default; debug video remains
...
| 4 | Flight log 기반 원인 분리와 테스트 추가 | `meta/events/frames`와 현장 영상·FC 로그 대조 |
| 5 | Safety monitor 실비행 임계값 검증 | vision stale, heartbeat, RC, battery, 상태별 timeout |
| 6 | Flight-log 전용 replay 도구 | 비행 후 자동 재현성과 보고서 자료 |
| 7 | GCS command channel 연동 | 현재 미구현; 필요성과 안전 설계를 먼저 확정 |
